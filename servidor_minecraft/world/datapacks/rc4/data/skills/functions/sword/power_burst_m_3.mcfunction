...
execute at @s run tag @e[type=#minecraft:hostile,limit=1,sort=nearest,distance=..1] add power_burst_target
...
execute at @s run execute as @e[type=#minecraft:hostile,nbt=!{HurtTime:10s},limit=1,sort=nearest,distance=..1] run damage @s 26 magic by @p[tag=power_burst_attack_2]