...
execute if entity @s[tag=!invulnerable,nbt=!{ActiveEffects:[{Id:11}]},scores={helper_health=..900}] run execute at @s run damage @s 9 magic by @p[scores={spell_cd=..4,last_spell=10}]
execute unless entity @s[tag=!invulnerable,nbt=!{ActiveEffects:[{Id:11}]},scores={helper_health=..900}] run execute at @s run damage @s 9 magic
