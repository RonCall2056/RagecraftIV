damage @s[tag=!no_target] 16 player_attack
scoreboard players set @s slice_5 10
tag @s add hurt
...
execute as @e[type=armor_stand,tag=vt_axe_throw,limit=1,sort=nearest] store result score @s[tag=!no_knockback] x run data get entity @s[tag=!no_knockback] Pos[0] 5
execute as @e[type=armor_stand,tag=vt_axe_throw,limit=1,sort=nearest] store result score @s[tag=!no_knockback] z run data get entity @s[tag=!no_knockback] Pos[2] 5
scoreboard players operation @s[tag=!no_knockback] x -= @e[type=armor_stand,tag=vt_axe_throw,limit=1,sort=nearest] x
scoreboard players set @s[tag=!no_knockback] y 3
scoreboard players operation @s[tag=!no_knockback] z -= @e[type=armor_stand,tag=vt_axe_throw,limit=1,sort=nearest] z
...
execute at @s run particle reverse_portal ~ ~1 ~ 0.3 0.5 0.3 0.01 80 normal
execute at @s run particle crit ~ ~1 ~ 0.3 0.5 0.3 0.01 40 normal
execute at @s run particle sweep_attack ~ ~1 ~ 0.3 0.3 0.3 0.2 3 normal

tag @s add vt_axe_throw_done