execute as @e[type=end_crystal,tag=arena_1_middle] at @s run execute as @p[distance=..70] at @s positioned ^ ^ ^3 run summon marker ~ ~5 ~ {Tags:["marker_tick","black_skill_marker","black_skill_3_21"],CustomName:'{"text":"✫ The Black Crystal ✫","color":"#533A69","bold":true,"italic":false}'}
