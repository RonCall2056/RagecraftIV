...
tellraw @a[x=188,y=1,z=1610,dx=170,dy=250,dz=140,predicate=general:dimension_abyss] [{"text":"The Black Crystal has empowered all Blackguards with ","color":"white","bold":false,"italic":false},{"text":"Dreadfire Weapons","color":"#E8915F","bold":true,"italic":false}]

execute as @e[type=husk,tag=blackguard,predicate=general:dimension_abyss,x=188,y=1,z=1610,dx=170,dy=250,dz=140] at @s run function boss:black/buff_blackguard_6a
