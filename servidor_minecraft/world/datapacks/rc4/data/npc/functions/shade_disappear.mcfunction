tag @s[tag=!shade_permanent] remove shade_visible
execute at @s[tag=!shade_permanent] run data modify entity @e[type=villager,tag=shade,limit=1,sort=nearest,distance=..10] Glowing set value false
execute at @s[tag=!shade_permanent] run data modify entity @e[type=villager,tag=shade,limit=1,sort=nearest,distance=..10] Silent set value true
