...
particle squid_ink ~ ~1 ~ 0.3 0.5 0.3 0.01 20 normal
...
execute at @s positioned ^ ^ ^9 run execute as @p[distance=..5] at @s run summon marker ^ ^1 ^-5 {NoGravity:1b,Silent:1b,Tags:["vt_trickster_marker"]}

execute at @s run summon marker ~ ~0.1 ~ {Tags:["vt_trickster_anim"]}
execute as @e[type=marker,tag=vt_trickster_anim] run function mobs:voidtouched/trickster_anim

tp @s @e[type=marker,tag=vt_trickster_marker,limit=1,sort=nearest]

execute at @s run summon marker ~ ~-0.9 ~ {Tags:["vt_trickster_anim"]}
execute as @e[type=marker,tag=vt_trickster_anim] run function mobs:voidtouched/trickster_anim
...
kill @e[type=marker,tag=vt_trickster_marker]