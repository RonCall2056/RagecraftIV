...
execute at @s run summon armor_stand ^ ^1 ^0.5 {NoGravity:1b,Silent:1b,Invulnerable:1b,Small:1b,Invisible:1b,NoBasePlate:1b,Tags:["beholder_ray_rotation","void","marker_tick","marker_tick_10"]}
...
execute at @s run execute as @e[type=armor_stand,tag=beholder_ray_2,limit=1,sort=nearest] run function mobs:voidtouched/beholder_ray_2